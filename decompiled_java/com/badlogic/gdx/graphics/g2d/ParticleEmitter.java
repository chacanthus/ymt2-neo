...
public class ParticleEmitter {
    ...
    static String readString(BufferedReader reader, String name) throws IOException {
        ...
        return v0.substring(v0.indexOf(":") + 1).trim();
    }
    ...
    private boolean updateParticle(Particle particle, float delta, int deltaMillis) {
        ...
        float v8;
        boolean v10;
        int v3 = particle.currentLife - deltaMillis;
        if(v3 <= 0) {
            v10 = false;
        }
        else {
            particle.currentLife = v3;
            float v4 = 1f - (((float)particle.currentLife)) / (((float)particle.life));
            int v6 = this.updateFlags;
            if((v6 & 1) != 0) {
                particle.setScale(particle.scale + particle.scaleDiff * this.scaleValue.getScale(v4));
            }

            if((v6 & 8) != 0) {
                float v7 = (particle.velocity + particle.velocityDiff * this.velocityValue.getScale(v4)) * delta;
                if((v6 & 2) != 0) {
                    float v1 = particle.angle + particle.angleDiff * this.angleValue.getScale(v4);
                    v8 = v7 * MathUtils.cosDeg(v1);
                    v9 = v7 * MathUtils.sinDeg(v1);
                    if((v6 & 4) != 0) {
                        v5 = particle.rotation + particle.rotationDiff * this.rotationValue.getScale(v4);
                        if(this.aligned) {
                            v5 += v1;
                        }

                        particle.setRotation(v5);
                    }
                }
                else {
                    v8 = v7 * particle.angleCos;
                    v9 = v7 * particle.angleSin;
                    if(!this.aligned && (v6 & 4) == 0) {
                        goto label_76;
                    }
            ...
            label_76:
                if((v6 & 16) != 0) {
                    v8 += (particle.wind + particle.windDiff * this.windValue.getScale(v4)) * delta;
                }

                if((v6 & 32) != 0) {
                    v9 += (particle.gravity + particle.gravityDiff * this.gravityValue.getScale(v4)) * delta;
                }

                particle.translate(v8, v9);
            }
            else {
                if((v6 & 4) == 0) {
                    goto label_104;
                ...
                particle.setRotation(particle.rotation + particle.rotationDiff * this.rotationValue.getScale(v4));
            }

        label_104:
            if((v6 & 64) != 0) {
                v2 = this.tintValue.getColor(v4);
            }
            else {
                v2 = particle.tint;
            }

            particle.setColor(v2[0], v2[1], v2[2], particle.transparency + particle.transparencyDiff * this.transparencyValue.getScale(v4));
            v10 = true;
        }
