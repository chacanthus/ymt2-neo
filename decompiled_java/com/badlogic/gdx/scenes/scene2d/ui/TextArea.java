...
public class TextArea extends TextField {
    ...
    protected void calculateOffsets() {
        ...
        if(!this.text.equals(this.lastText)) {
            ...
            TextBounds v0 = new TextBounds();
            BitmapFont v1 = this.style.font;
            float v8 = this.getWidth();
            if(this.style.background != null) {
                v7 = this.style.background.getLeftWidth() + this.style.background.getRightWidth();
            }
            else {
                v7 = 0f;
            }
            ...
            int v4 = 0;
            int v2;
            for(v2 = 0; v2 < this.text.length(); ++v2) {
                int v3 = this.text.charAt(v2);
                if(v3 == 13 || v3 == 10) {
                    this.linesBreak.add(v5);
                    this.linesBreak.add(v2);
                    v5 = v2 + 1;
                }
                else {
                    if(!this.continueCursor(v2, 0)) {
                        v4 = v2;
                    }

                    v1.getBounds(this.text, v5, v2 + 1, v0);
                    if(v0.width <= v6) {
                        goto label_41;
                    }
    ...
    protected boolean continueCursor(int index, int offset) {
        ...
        if(super.continueCursor(index, offset)) {
            if(v0 >= 0 && v0 < this.linesBreak.size && this.linesBreak.items[v0 + 1] == index && this.linesBreak.items[v0 + 1] != this.linesBreak.items[v0 + 2]) {
                goto label_24;
            }
    ...
    protected void drawText(Batch batch, BitmapFont font, float x, float y) {
        float v8 = 0f;
        int v7 = this.firstLineShowing * 2;
        while(v7 < (this.firstLineShowing + this.linesShowing) * 2) {
            if(v7 >= this.linesBreak.size) {
                return;
            }

            font.draw(batch, this.displayText, x, y + v8, this.linesBreak.items[v7], this.linesBreak.items[v7 + 1]);
            v8 -= font.getLineHeight();
            v7 += 2;
        }
    }
