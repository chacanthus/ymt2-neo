...
public class ScrollPane extends WidgetGroup {
    ...
    public void act(float delta) {
        ...
        float v8 = 5f;
        super.act(delta);
        boolean v1 = this.flickScrollListener.getGestureDetector().isPanning();
        if(this.fadeAlpha > 0f && (this.fadeScrollBars) && !v1 && !this.touchScrollH && !this.touchScrollV) {
    ...
    label_73:
        if(!this.smoothScrolling || this.flingTimer > 0f || (this.touchScrollH) || (this.touchScrollV) || (v1)) {
            ...
            if(this.visualAmountX != this.amountX) {
                if(this.visualAmountX < this.amountX) {
                    this.visualScrollX(Math.min(this.amountX, this.visualAmountX + Math.max(v9 * delta, (this.amountX - this.visualAmountX) * v8 * delta)));
                }
                else {
                    this.visualScrollX(Math.max(this.amountX, this.visualAmountX - Math.max(v9 * delta, (this.visualAmountX - this.amountX) * v8 * delta)));
                }
            }
            ...
            this.visualScrollY(Math.min(this.amountY, this.visualAmountY + Math.max(v9 * delta, (this.amountY - this.visualAmountY) * v8 * delta)));
            goto label_118;
        label_181:
            this.visualScrollY(Math.max(this.amountY, this.visualAmountY - Math.max(v9 * delta, (this.visualAmountY - this.amountY) * v8 * delta)));
        }
    ...
    void cancelTouchFocusedChild(InputEvent event) {
        if(this.cancelTouchFocus) {
            ...
            if(v0 != null) {
                v0.cancelTouchFocus(this.flickScrollListener, ((Actor)this));
            }
        }
    }
    ...
    public void draw(Batch batch, float parentAlpha) {
        if(this.widget != null) {
            ...
            float v9 = this.widgetAreaBounds.y;
            if(!this.scrollY) {
                v9 -= ((float)(((int)this.maxY)));
            }
            else {
                v9 -= ((float)(((int)(this.maxY - this.visualAmountY))));
            }
            ...
            float v8 = this.widgetAreaBounds.x;
            if(this.scrollX) {
                v8 -= ((float)(((int)this.visualAmountX)));
            }

            this.widget.setPosition(v8, v9);
    ...
    public void scrollToCenter(float x, float y, float width, float height) {
        ...
        this.scrollX(MathUtils.clamp(v0, 0f, this.maxX));
        float v1 = this.amountY;
        float v2 = this.maxY - y + this.areaHeight / v6 - height / v6;
        if(v1 < v2 - this.areaHeight / v7 || v1 > this.areaHeight / v7 + v2) {
            v1 = v2;
        }
