...
public class Dialog extends Window {
    ...
    label_27:
        if(Dialog.fadeDuration > 0f) {
            ...
            this.addAction(Actions.sequence(Actions.fadeOut(Dialog.fadeDuration, Interpolation.fade), Actions.removeListener(this.ignoreTouchDown, true), Actions.removeActor()));
        }
        else {
            this.remove();
        }
        ...
        this.addListener(new FocusListener() {
            private void focusChanged(FocusEvent event) {
                ...
                if((Dialog.this.isModal) && v1 != null && v1.getRoot().getChildren().size > 0 && v1.getRoot().getChildren().peek() == Dialog.this) {
                    ...
                    if(v0 != null && !v0.isDescendantOf(Dialog.this)) {
                        event.cancel();
                    }
                }
            }
    ...
    public Dialog show(Stage stage) {
        ...
        this.pack();
        this.setPosition(((float)Math.round((stage.getWidth() - this.getWidth()) / v5)), ((float)Math.round((stage.getHeight() - this.getHeight()) / v5)));
