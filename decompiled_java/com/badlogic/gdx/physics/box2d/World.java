...
public final class World implements Disposable {
    ...
    public Body createBody(BodyDef def) {
        long v20 = this.jniCreateBody(this.addr, def.type.getValue(), def.position.x, def.position.y, def.angle, def.linearVelocity.x, def.linearVelocity.y, def.angularVelocity, def.linearDamping, def.angularDamping, def.allowSleep, def.awake, def.fixedRotation, def.bullet, def.active, def.gravityScale);
        Object v19 = this.freeBodies.obtain();
        ((Body)v19).reset(v20);
        this.bodies.put(v19.addr, v19);
        return ((Body)v19);
    }

    public Joint createJoint(JointDef def) {
        RevoluteJoint v0_6;
        long v1 = this.createProperJoint(def);
        DistanceJoint v0 = null;
        if(def.type == JointType.DistanceJoint) {
            v0 = new DistanceJoint(this, v1);
        }

        if(def.type == JointType.FrictionJoint) {
            FrictionJoint v0_1 = new FrictionJoint(this, v1);
        }

        if(def.type == JointType.GearJoint) {
            GearJoint v0_2 = new GearJoint(this, v1);
        }

        if(def.type == JointType.MouseJoint) {
            MouseJoint v0_3 = new MouseJoint(this, v1);
        }

        if(def.type == JointType.PrismaticJoint) {
            PrismaticJoint v0_4 = new PrismaticJoint(this, v1);
        }

        if(def.type == JointType.PulleyJoint) {
            PulleyJoint v0_5 = new PulleyJoint(this, v1);
        }

        if(def.type == JointType.RevoluteJoint) {
            v0_6 = new RevoluteJoint(this, v1);
        }

        if(def.type == JointType.WeldJoint) {
            WeldJoint v0_7 = new WeldJoint(this, v1);
        }

        if(def.type == JointType.RopeJoint) {
            RopeJoint v0_8 = new RopeJoint(this, v1);
        }

        if(def.type == JointType.WheelJoint) {
            WheelJoint v0_9 = new WheelJoint(this, v1);
        }

        if((((DistanceJoint)v0_6)) != null) {
            this.joints.put(((Joint)v0_6).addr, v0_6);
        }

        JointEdge v3 = new JointEdge(def.bodyB, ((Joint)v0_6));
        JointEdge v4 = new JointEdge(def.bodyA, ((Joint)v0_6));
        ((Joint)v0_6).jointEdgeA = v3;
        ((Joint)v0_6).jointEdgeB = v4;
        def.bodyA.joints.add(v3);
        def.bodyB.joints.add(v4);
        return ((Joint)v0_6);
    }
    ...
    private long createProperJoint(JointDef def) {
        long v1;
        if(def.type == JointType.DistanceJoint) {
            v1 = this.jniCreateDistanceJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.length, def.frequencyHz, def.dampingRatio);
        }
        else if(def.type == JointType.FrictionJoint) {
            v1 = this.jniCreateFrictionJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.maxForce, def.maxTorque);
        }
        else if(def.type == JointType.GearJoint) {
            v1 = this.jniCreateGearJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.joint1.addr, def.joint2.addr, def.ratio);
        }
        else if(def.type == JointType.MouseJoint) {
            v1 = this.jniCreateMouseJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.target.x, def.target.y, def.maxForce, def.frequencyHz, def.dampingRatio);
        }
        else if(def.type == JointType.PrismaticJoint) {
            v1 = this.jniCreatePrismaticJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.localAxisA.x, def.localAxisA.y, def.referenceAngle, def.enableLimit, def.lowerTranslation, def.upperTranslation, def.enableMotor, def.maxMotorForce, def.motorSpeed);
        }
        else if(def.type == JointType.PulleyJoint) {
            v1 = this.jniCreatePulleyJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.groundAnchorA.x, def.groundAnchorA.y, def.groundAnchorB.x, def.groundAnchorB.y, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.lengthA, def.lengthB, def.ratio);
        }
        else if(def.type == JointType.RevoluteJoint) {
            v1 = this.jniCreateRevoluteJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.referenceAngle, def.enableLimit, def.lowerAngle, def.upperAngle, def.enableMotor, def.motorSpeed, def.maxMotorTorque);
        }
        else if(def.type == JointType.WeldJoint) {
            v1 = this.jniCreateWeldJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.referenceAngle);
        }
        else if(def.type == JointType.RopeJoint) {
            v1 = this.jniCreateRopeJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.maxLength);
        }
        else if(def.type == JointType.WheelJoint) {
            v1 = this.jniCreateWheelJoint(this.addr, def.bodyA.addr, def.bodyB.addr, def.collideConnected, def.localAnchorA.x, def.localAnchorA.y, def.localAnchorB.x, def.localAnchorB.y, def.localAxisA.x, def.localAxisA.y, def.enableMotor, def.maxMotorTorque, def.motorSpeed, def.frequencyHz, def.dampingRatio);
        }
        else {
            v1 = 0;
        }
    ...
    private native long jniCreateWeldJoint(long arg1, long arg2, boolean arg3, float arg4, float arg5, float arg6, float arg7, float arg8) {
    }
