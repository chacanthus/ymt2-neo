...
public class Lzma {
    ...
    public static void compress(InputStream in, OutputStream out) throws IOException {
        ...
        Encoder v0 = new Encoder();
        if(!v0.SetAlgorithm(v12.Algorithm)) {
            throw new RuntimeException("Incorrect compression mode");
        }

        if(!v0.SetDictionarySize(v12.DictionarySize)) {
            throw new RuntimeException("Incorrect dictionary size");
        }

        if(!v0.SetNumFastBytes(v12.Fb)) {
            throw new RuntimeException("Incorrect -fb value");
        }

        if(!v0.SetMatchFinder(v12.MatchFinder)) {
            throw new RuntimeException("Incorrect -mf value");
        }

        if(!v0.SetLcLpPb(v12.Lc, v12.Lp, v12.Pb)) {
            throw new RuntimeException("Incorrect -lc or -lp or -pb value");
        }

        v0.SetEndMarkerMode(v8);
        v0.WriteCoderProperties(out);
        if(v8) {
            v9 = -1;
        }
        else {
            v9 = ((long)in.available());
            if(v9 == 0) {
                v9 = -1;
            }
        }
        ...
        int v11;
        for(v11 = 0; v11 < 8; ++v11) {
            out.write((((int)(v9 >>> v11 * 8))) & 255);
        }

        v0.Code(in, out, v3, v3, null);
    }
