...
public class EarClippingTriangulator {
    ...
    private static boolean areVerticesClockwise(float[] vertices, int offset, int count) {
        ...
        if(count > 2) {
            ...
            if(vertices[count - 2] * vertices[1] + v0 - vertices[0] * vertices[count - 1] >= 0f) {
                v7 = false;
            }
    ...
    public ShortArray computeTriangles(float[] vertices, int offset, int count) {
        ...
        if(EarClippingTriangulator.areVerticesClockwise(vertices, offset, count)) {
            ...
            v3 = v5 - 1;
            while(v0_1 < v5) {
                v1[v0_1] = ((short)(v3 - v0_1));
                ++v0_1;
            }
        }

        IntArray v6 = this.vertexTypes;
        v6.clear();
        v6.ensureCapacity(v5);
        v0_1 = 0;
        v3 = v5;
        while(v0_1 < v3) {
            v6.add(this.classifyVertex(v0_1));
            ++v0_1;
        }
