...
public class ConvexHull {
    ...
    public FloatArray computePolygon(FloatArray points, boolean sorted, boolean yDown) {
        return this.computePolygon(points.items, 0, points.size, sorted, yDown);
    }
    ...
    public FloatArray computePolygon(float[] polygon, boolean sorted, boolean yDown) {
        return this.computePolygon(polygon, 0, polygon.length, sorted, yDown);
    }

    private int quicksortPartition(float[] values, int lower, int upper, boolean yDown) {
    ...
    private void sort(float[] values, int count, boolean yDown) {
        ...
        while(true) {
            ...
            int v0 = this.quicksortPartition(values, v1, v3, yDown);
            if(v0 - v1 > v3 - v0) {
                v2.add(v1);
                v2.add(v0 - 2);
            }
