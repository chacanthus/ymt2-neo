...
class MyTask extends TimerTask {
    ...
    public void run() {
        int v4 = 5;
        ++Global.iLiveTick;
        if(Global.reserve[0] > 1) {
            --Global.reserve[0];
        }
        else if(Global.reserve[0] == 1) {
            ++GameScreen.iReserveConfirmTick[0];
            if(GameScreen.iReserveConfirmTick[0] >= v4) {
                GameScreen.iReserveConfirmTick[0] = 0;
                GameScreen.socketWrite("G110\t" + Global.id + "\t" + "0" + "\t" + String.valueOf(Global.machineNo[0]));
            }
        }

        if(Global.reserve[1] > 1) {
            --Global.reserve[1];
        }
        else if(Global.reserve[1] == 1) {
            ++GameScreen.iReserveConfirmTick[1];
            if(GameScreen.iReserveConfirmTick[1] >= v4) {
                GameScreen.iReserveConfirmTick[1] = 0;
                GameScreen.socketWrite("G110\t" + Global.id + "\t" + "1" + "\t" + String.valueOf(Global.machineNo[1]));
            }
        }
        ...
        if(Global.channel == 0) {
            if(Global.reserve[0] <= 1) {
                ...
                GameScreen.lbReserve.setText(this.toTime(Global.reserve[0]));
            }
        }
            ...
            if(Global.reserve[1] <= 1) {
                ...
                GameScreen.lbReserve.setText(this.toTime(Global.reserve[1]));
            }
        }
